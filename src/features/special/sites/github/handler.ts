...
import { GitHubURL, GitHubURLSchema } from './types';
  ...
  parse(url: string): GitHubURL {
    ...
    switch (segments[2]) {
      ...
      case 'tree':
        ...
        if (segments[2] === 'blob' && parsedUrl.hash) {
          ...
          if (lineMatch) {
            ...
            if (lineMatch[2]) {
              result.lineEnd = parseInt(lineMatch[2], 10);
            }
          }
        }
        break;
      ...
      case 'issues':
        ...
        if (segments.length === 4) {
          result.issueNumber = parseInt(segments[3], 10);
        }
        break;
      ...
      case 'pull':
        ...
        result = {
          ...result,
          type: 'pull',
          pullNumber: parseInt(segments[3], 10)
        };
        break;
